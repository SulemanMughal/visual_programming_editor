...
type RowProps = {
  ...
  onToggleSelect: (id: string, nextVal: boolean) => void;
...
const Row = React.memo(function Row({
  node: n, isOpen, isSelected, isIndeterminate, onToggle, onToggleSelect, style, q, isMatch
}: RowProps) {
  const id = n.path.join(".") || "root";
  const isBranch = n.type === "object" || n.type === "array";

  // Selected row = blue background; otherwise white
  const rowBG = isSelected ? "bg-blue-500" : "bg-white";
  const textMain = isSelected ? "text-white" : "text-gray-900";
  const textSub  = isSelected ? "text-white/90" : "text-gray-700";
  const iconTone = isSelected ? "text-white" : (isBranch ? "text-blue-600" : "text-gray-500");

  // Button state & label
  const nextVal = !isSelected || isIndeterminate; // if partial, clicking selects all
  const btnLabel = isIndeterminate ? "Select all" : (isSelected ? "Selected" : "Select");
    ...
    <div style={style}>
      ...
      >
        ...
        <div className="flex items-center h-10 px-4 gap-2" role="cell">
          ...
          <button
            type="button"
            onClick={() => onToggleSelect(id, nextVal)}
            className={[
            ...
            title={isIndeterminate ? "Some children selected — click to select all" : ""}
          >
            {btnLabel}
          </button>

          <span className={`truncate font-medium ${isMatch && !isSelected ? "text-blue-700" : textMain}`}>
...
const RowRenderer = ({ index, style, data }: ListChildComponentProps<ItemData>) => {
  ...
  const id = n.path.join(".") || "root";
  const isSelected = data.checked.has(id);
  const isInd = data.isIndeterminate(id);
...
/* ===================== Main component (button selection) ===================== */
export default function JsonTreeGridVirtButton({ json }: { json: J }) {
  const tree = useMemo(() => buildTree(json), [json]);
  const allNodes = useMemo(() => collectAll(tree), [tree]);

  // Quick lookup: id -> node (subtree traversal)
  ...
  // Toggle selection as a BUTTON (cascades to subtree)
  const onToggleSelect = useCallback((id: string, nextVal: boolean) => {
    setChecked((prev) => {
      const next = new Set(prev);
      const ids = collectSubtreeIds(id);
      if (nextVal) {
  ...
  // Chips
  const removeKey = useCallback((k: string) => onToggleSelect(k, false), [onToggleSelect]);
  ...
  const itemData = useMemo(() => ({
    ...
    matches,
    isIndeterminate,
  }), [visible, expandedEffective, checked, onToggle, onToggleSelect, qd, matches, isIndeterminate]);
